function calculadora_de_cadenas(cadena) {
    ...
    const escapeRegularExpressions = (valor) => valor
    .replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    ...
    if (encabezado) {
        const delimitadoresPersonalizados = [...encabezado[1]
        .matchAll(/\[([^\]]+)\]/g)]
        .map((coincidencia) => coincidencia[1]);

        separadores
        .push(...delimitadoresPersonalizados);

        contenido = encabezado[2];
    }

    const patronSeparadores = new RegExp(separadores
        .map(escapeRegularExpressions)
        .join('|'));
